...
class LoginController extends Controller
{
     ...
     *
     * @OA\Post(
     *      path="/api/user/v1/auth/login", tags = {"前端-用户"}, summary="用户账号密码登录",
     *      @OA\Parameter(
    ...
    public function login(Request $request)
    {
        ...
        if ($request->has('phone')) {
            ...
            $res = SmsFactory::make()->checkCode($phone, $sms_code);
            if ($res !== true) {
            ...
            if (! $user) {
                $user = $this->create($phone, $referrer);
            }
        } else {
     ...
     *
     * @OA\Post(
     *      path="/api/user/v1/auth/refresh/token", tags = {"后端-用户"}, summary="用户刷新 Token",
     *      @OA\Parameter(
     *          in="header", name="Authorization", description="用户令牌", required=true,
     *          @OA\Schema(type="string", example="Bearer b43c5b13522193473deb3f37a3c2f50cbc5e14b6")
     *      ),
     ...
     * @param $referrer
     */
    protected function create($phone, $referrer)
    {
        $referrer = User::where('id', $referrer)->first();
